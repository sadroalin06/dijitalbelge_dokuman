...
```javascript
async function signLoginChallenge(ws, pin, slot, loginTxId, challenge, istest = true) {
  return new Promise((resolve, reject) => {
    ...
    const timeout = setTimeout(() => {
      reject(new Error("signLogin işlemi zaman aşımına uğradı"));
    }, 30000);
    ...
    const messageHandler = (event) => {
      ...
      if (response.requestId === requestId && response.type === "signLogin") {
...
async function loginWithSignature() {
  ...
  ws.onopen = async () => {
    try {
      // 1. E-İmza Login API'sinden challenge al
      const loginTxResponse = await fetch(
        "https://api.dijitalbelge.com/api/external/auth/tx",
        {
          method: "POST",
          headers: {
      ...
      const { loginTxId, challenge } = await loginTxResponse.json();
      ...
      if (signResponse.data && signResponse.data.success) {
        console.log("İmzalama başarılı:", signResponse.data.signature);
        
        // 3. Token al
        const tokenResponse = await fetch(
          `https://api.dijitalbelge.com/api/external/auth/tx/${loginTxId}/token`,
          {
            method: "POST",
            headers: {
              "X-Client-Id": "your-client-id",
        ...
        const { loginToken, user } = await tokenResponse.json();
        console.log("Giriş başarılı:", user);
      } else {
        console.error("İmzalama başarısız:", signResponse.error);
      }
    } catch (error) {
      console.error("Hata:", error);
    }
